...
from src.models.service_quote import ServiceType, PropertyType, PerthSuburb, QuoteStatus
...
class ServiceQuoteRequest(BaseModel):
    ...
    window_count: Optional[int] = Field(None, ge=1, le=200, description="Number of windows")
    square_meters: Optional[float] = Field(None, ge=1, le=10000, description="Area in square meters")
    ...
    customer_notes: Optional[str] = Field(None, max_length=1000, description="Additional notes")
...
async def create_quote_from_voice(
    ...
    try:
        quote_data = service.create_quote_from_voice(
            user_id=current_user.id,
            voice_recording_id=request.voice_recording_id
        )
...
async def get_quote_analytics(
    ...
    """
    analytics = service.get_quote_analytics(
        user_id=current_user.id,
        days=days
    )
