...
class AnalyticsEvent(Base):
    ...
    session_id = Column(GUID(), ForeignKey("user_sessions.id"), nullable=True)
...
class UserSession(Base):
    ...
    def end_session(self) -> None:
        ...
        self.engaged = (
            self.page_views > 2
            or (self.duration_seconds or 0) > 120
            or self.quotes_generated > 0
            or self.voice_recordings > 0
        )
...
class FunnelStep(Base):
    ...
    def __repr__(self) -> str:
        return (
            f"<FunnelStep(funnel_name={self.funnel_name}, step_name={self.step_name})>"
        )
