...
from .monitoring.tracing import (
    get_tracer, get_meter, trace_ai_operation, trace_ai_request, 
    add_trace_attributes, record_exception
)
from .monitoring.circuit_breaker import (
    circuit_breaker_manager, with_circuit_breaker, with_intelligent_retry,
...
from .monitoring.smart_routing import (
    ...
    with_smart_routing, NoProvidersAvailableException
)
...
class EnhancedAIService:
    ...
    def _setup_metrics(self):
        ...
        self.response_time_histogram = self.metrics.create_histogram(
            ...
            description="AI request response times",
            unit="s"
        )
    ...
    async def generate_quote(self, request: AIRequest) -> AIResponse:
        ...
        with trace_ai_operation(
            ...
            "generation",
            {
                "request.category": request.category.value if request.category else None,
            ...
            try:
                ...
                self.response_time_histogram.record(response.response_time, {
                    "provider": provider_name
                })
                self.token_usage_counter.add(response.tokens_used, {
                    "provider": provider_name
                })
                ...
                fallback_response = await self._handle_infrastructure_failure(request, request_id, start_time)
    ...
    async def _call_openai(self, request: AIRequest, request_id: str) -> AIResponse:
        ...
        try:
            ...
            return AIResponse(
                ...
                routing_metadata={
                    "provider_latency": response.usage.total_tokens / 1000  # Rough estimate
                }
            )
    ...
    async def _call_anthropic(self, request: AIRequest, request_id: str) -> AIResponse:
        ...
        try:
            ...
            messages = [{"role": "user", "content": self._build_anthropic_prompt(request)}]
    ...
    @with_intelligent_retry(max_attempts=3)
    async def _call_azure_openai(self, request: AIRequest, request_id: str) -> AIResponse:
    ...
    def _get_system_prompt(self, category: Optional[ServiceCategory], tone: str) -> str:
        ...
        if category:
            ...
            base_prompt += category_prompts.get(
                category, 
                "Focus on general cleaning and maintenance services."
            )
