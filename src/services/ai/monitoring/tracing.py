...
class AIServiceTracing:
    ...
    def _instrument_libraries(self) -> None:
        ...
        try:
            ...
            logger.info(
                "Automatic instrumentation enabled for HTTP and database operations"
            )
...
def trace_ai_operation(
    ...
    with tracer.start_as_current_span(
        f"ai.{operation_name}", attributes=span_attributes
    ) as span:
    ...
    async def async_wrapper(*args, **kwargs):
        ...
        with trace_ai_operation(operation_name, provider, model, attributes) as span:
            ...
            if hasattr(result, "usage"):
                span.set_attribute(
                    "ai.response.tokens.total", getattr(result.usage, "total_tokens", 0)
                )
                span.set_attribute(
                    "ai.response.tokens.prompt",
                    getattr(result.usage, "prompt_tokens", 0),
                )
                span.set_attribute(
                    "ai.response.tokens.completion",
