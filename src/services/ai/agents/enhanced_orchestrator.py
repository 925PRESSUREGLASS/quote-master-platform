...
from ..orchestrator import AIOrchestrator, RoutingStrategy as BaseRoutingStrategy
from ..enhanced_ai_service import EnhancedAIService, AIRequest as BaseAIRequest, AIResponse, get_ai_service
from .agent_types import (
    AgentRole, AgentCapability, AgentContext, TaskComplexity, 
    ProviderTier, AGENT_CAPABILITIES, PROVIDER_MODEL_MAPPING, 
...
class AgentRequest(BaseAIRequest):
    ...
    def __init__(self, prompt: str, agent_role: AgentRole, context: AgentContext, **kwargs):
...
class EnhancedAIOrchestrator:
    ...
    async def execute_agent_task(self, request: AgentRequest) -> Dict[str, Any]:
        ...
        try:
            ...
            optimized_request = BaseAIRequest(
                ...
                preferred_provider=self._get_provider_from_key(optimization_result["provider_key"])
            )
            ...
            return {
                ...
                "metrics": {
                    ...
                    "token_efficiency": optimization_result["optimization_metadata"]["compression_ratio"]
                },
                "agent_info": {
                    "role": request.agent_role.value,
                    "capabilities": [cap.value for cap in AGENT_CAPABILITIES[request.agent_role].capabilities],
    ...
    async def execute_multi_agent_workflow(self, workflow_request: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for step_index, step in enumerate(workflow_steps):
                ...
                if step_result["success"]:
                    ...
                    self.workflow_metrics[workflow_id]["total_cost"] += step_result["metrics"]["cost_usd"]
                    self.workflow_metrics[workflow_id]["total_tokens"] += step_result["metrics"]["tokens_used"]
                    self.workflow_metrics[workflow_id]["agents_used"].append(step["agent_role"])
                    
                    # Update accumulated context for next step
                    accumulated_context.update({
                        f"step_{step_index}_result": step_result["response"]["text"],
                        f"step_{step_index}_quality": step_result["response"]["quality_score"]
                    })
                else:
            ...
            workflow_success = len([r for r in workflow_results if r["success"]]) == len(workflow_steps)
            ...
            return {
                ...
                "metrics": {
                    ...
                    "steps_completed": self.workflow_metrics[workflow_id]["completed_steps"],
                    ...
                    "avg_cost_per_step": self.workflow_metrics[workflow_id]["total_cost"] / len(workflow_steps),
                    "cost_efficiency": self._calculate_workflow_cost_efficiency(workflow_id)
                }
            }
    ...
    async def _update_agent_metrics(self, agent_role: AgentRole, usage_stats: Optional[TokenUsageStats], success: bool):
    ...
    async def get_agent_recommendations(self, task_description: str) -> List[Dict[str, Any]]:
        ...
        if any(word in task_lower for word in ["generate", "create", "write", "compose"]):
            recommendations.append({
        ...
        if any(word in task_lower for word in ["voice", "audio", "transcribe", "speech"]):
            recommendations.append({
        ...
        if any(word in task_lower for word in ["psychology", "emotion", "personality", "feeling"]):
            recommendations.append({
        ...
        if any(word in task_lower for word in ["important", "critical", "final", "publish"]):
            recommendations.append({
    ...
    async def get_orchestrator_status(self) -> Dict[str, Any]:
        ...
        ai_service_health = await self.enhanced_ai_service.get_health_status() if self.enhanced_ai_service else {}
        ...
        total_agent_requests = sum(metrics["total_requests"] for metrics in self.agent_metrics.values())
        agent_utilization = {
            role.value: {
