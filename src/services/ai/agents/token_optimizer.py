...
from .agent_types import (
    AgentRole, TaskComplexity, ProviderTier, 
    PROVIDER_MODEL_MAPPING, get_optimal_provider
)
...
class TokenOptimizer:
    ...
    def set_strategy(self, strategy: TokenStrategy, custom_rules: Optional[Dict] = None):
    ...
    async def optimize_request(self, agent_role: AgentRole, prompt: str, 
        ...
        estimated_cost = (estimated_tokens / 1000) * provider_perf.cost_per_1k_tokens if provider_perf else 0.01
        ...
        return {
            ...
            "optimization_metadata": {
                ...
                "compression_ratio": len(optimized_prompt) / len(prompt) if prompt else 1.0,
                "selected_tier": optimal_tier.value,
                "strategy": self.strategy.value
            }
        }
    
    async def _select_optimal_provider(self, preferred_tier: ProviderTier, 
        ...
        if not candidates:
            ...
            candidates = [(k, p, (estimated_tokens / 1000) * p.cost_per_1k_tokens) 
    ...
    def _score_candidates(self, candidates: List[Tuple[str, ProviderPerformance, float]]) -> Optional[Tuple[str, ProviderPerformance, float]]:
    ...
    def _estimate_tokens(self, prompt: str, context: Optional[str] = None, 
                        max_tokens: int = 500) -> int:
    ...
    def _optimize_prompt(self, prompt: str, provider_key: str, 
    ...
    async def record_usage(self, provider_key: str, usage_stats: TokenUsageStats):
        ...
        if provider_key in self.performance_cache:
            ...
            perf.avg_cost_per_request = (
                (1 - alpha) * perf.avg_cost_per_request +
    ...
    def get_recommendations(self) -> List[str]:
        ...
        if avg_tokens > 800:
            recommendations.append(
                "Enable prompt compression to reduce token usage"
            )
