...
from .agent_types import AGENT_CAPABILITIES, AgentRole, ProviderTier, TaskComplexity
...
class AgentMonitor:
    ...
    def __init__(self, max_history_size: int = 10000):
        ...
        self.provider_usage: Dict[str, Dict[str, Any]] = defaultdict(
            lambda: {
                "requests": 0,
                "tokens": 0,
                "cost": 0.0,
    ...
    async def complete_request_tracking(
        ...
        final_quality_score = quality_score or (
            usage_stats.quality_score if usage_stats else 0.0
        )
        ...
        self._update_provider_metrics(
            provider_key, success, response_time_ms, actual_cost, tokens_used
        )
    ...
    async def _update_agent_metrics(
        ...
        if success:
            ...
            metrics.avg_response_time_ms = (
                1 - alpha
            ) * metrics.avg_response_time_ms + alpha * response_time_ms

            metrics.avg_cost_per_request = (
                1 - alpha
            ) * metrics.avg_cost_per_request + alpha * cost

            metrics.avg_tokens_per_request = (
                1 - alpha
            ) * metrics.avg_tokens_per_request + alpha * tokens

            metrics.avg_quality_score = (
                1 - alpha
            ) * metrics.avg_quality_score + alpha * quality_score
            ...
            provider = (
                provider_key.split(":")[0] if ":" in provider_key else provider_key
            )
            ...
            if tokens > 0:
                ...
                metrics.token_efficiency_score = (
                    1 - alpha
                ) * metrics.token_efficiency_score + alpha * efficiency
    ...
    def _update_provider_metrics(
        ...
        if success:
            # Update averages
            alpha = 0.1
            provider_stats["avg_response_time"] = (1 - alpha) * provider_stats[
                "avg_response_time"
            ...
            # Update success rate
            current_successes = provider_stats["success_rate"] * (
                provider_stats["requests"] - 1
            )
            provider_stats["success_rate"] = (current_successes + 1) / provider_stats[
                "requests"
            ]
        else:
            # Update failure rate
            ...
            provider_stats["success_rate"] = (
                current_successes / provider_stats["requests"]
            )
    ...
    async def get_agent_performance(self, agent_role: AgentRole) -> Dict[str, Any]:
        ...
        return {
            ...
            "quality_metrics": {
                ...
                "quality_trend": metrics.quality_scores[-10:]
            ...
            "time_span": {
                "first_request": metrics.first_request_time.isoformat()
    ...
    async def get_system_health(self) -> Dict[str, Any]:
        ...
        total_successful = sum(
            m.successful_requests for m in self.agent_metrics.values()
        )
        ...
        alerts = self._check_alert_conditions(
            avg_response_time, overall_success_rate, total_cost
        )
        ...
        for provider_key, stats in self.provider_usage.items():
            if stats["requests"] > 0:
                health_score = stats["success_rate"] * (
                    1 - min(stats["avg_response_time"] / 10000, 1)
                )
                if health_score > 0.8:
                    status = "healthy"
        ...
        health_data = {
            ...
            "summary": {
                "total_active_agents": len(
                    [m for m in self.agent_metrics.values() if m.active_requests > 0]
                ),
    ...
    def _check_alert_conditions(
        self, avg_response_time: float, success_rate: float, total_cost: float
    ) -> List[str]:
    ...
    def _get_top_performers(self, limit: int) -> List[Dict[str, Any]]:
        ...
        return sorted(performers, key=lambda x: x["overall_score"], reverse=True)[
            :limit
        ]
    ...
    def _get_cost_recommendations(
        self, agent_costs: Dict, provider_costs: Dict
    ) -> List[str]:
    ...
    async def export_metrics(self, format_type: str = "json") -> str:
        ...
        export_data = {
            ...
            "request_history_sample": list(self.request_history)[
                -100:
            ],  # Last 100 requests
        ...
        for role, metrics in self.agent_metrics.items():
            export_data["agent_metrics"][role.value] = await self.get_agent_performance(
                role
            )
