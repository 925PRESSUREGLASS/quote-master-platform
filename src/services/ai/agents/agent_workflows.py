...
from .agent_types import AgentRole, AgentContext, TaskComplexity
from .enhanced_orchestrator import EnhancedAIOrchestrator, AgentRequest
...
class AgentWorkflowManager:
    ...
    def _initialize_builtin_workflows(self):
        ...
        self.workflow_definitions[code_development_workflow.workflow_id] = code_development_workflow
        self.workflow_definitions[api_integration_workflow.workflow_id] = api_integration_workflow
    ...
    async def register_workflow(self, workflow: WorkflowDefinition) -> bool:
        ...
        try:
            ...
            logger.info(f"Registered workflow: {workflow.name} (ID: {workflow.workflow_id})")
            return True
        ...
        for step in workflow.steps:
            ...
            if step.step_type == WorkflowStepType.AGENT_TASK:
                ...
                if not step.prompt_template:
                    errors.append(f"Agent task step {step.step_id} missing prompt_template")
    ...
    async def _execute_steps_with_dependencies(self, execution: WorkflowExecution,
        ...
        while len(completed_steps) < len(workflow.steps):
            ...
            if not ready_steps and not running_tasks:
                ...
                raise RuntimeError(f"Circular dependency or unresolvable dependencies: {remaining_steps}")
                ...
                for task in done_tasks:
                    ...
                    if completed_step_id:
                        try:
                            ...
                            logger.debug(f"Step {completed_step_id} completed in execution {execution.execution_id}")
                            
                        except Exception as e:
                            execution.failed_steps.append(completed_step_id)
                            execution.error_log.append(f"Step {completed_step_id} failed: {str(e)}")
                            ...
                            if has_dependents:
                                raise RuntimeError(f"Critical step {completed_step_id} failed, aborting workflow")
                            
                            logger.warning(f"Non-critical step {completed_step_id} failed, continuing")
    ...
    async def _execute_agent_task_step(self, execution: WorkflowExecution, step: WorkflowStep) -> Dict[str, Any]:
    ...
    async def _execute_transform_step(self, execution: WorkflowExecution, step: WorkflowStep) -> Dict[str, Any]:
    ...
    def _render_prompt_template(self, template: str, execution: WorkflowExecution) -> str:
    ...
    async def get_workflow_analytics(self, workflow_id: str, days: int = 30) -> Dict[str, Any]:
        ...
        relevant_executions = [
            ...
            if e.workflow_id == workflow_id and e.start_time and e.start_time >= cutoff_date
        ]
        
        if not relevant_executions:
            return {"message": f"No executions found for {workflow_id} in the last {days} days"}
        ...
        # Duration analytics
        completed_executions = [e for e in relevant_executions if e.end_time and e.start_time]
...
def get_workflow_manager(orchestrator: Optional[EnhancedAIOrchestrator] = None) -> AgentWorkflowManager:
