...
from .base import AIServiceBase, AIRequest, AIResponse, AITaskType, AIModelType, AIServiceError
...
class AIOrchestrator:
    ...
    def _initialize_services(self) -> None:
        ...
        try:
            ...
            if settings.openai_api_key:
                self.services["openai_gpt4"] = OpenAIService(
                    model_name="gpt-4-turbo-preview"
                )
                self.services["openai_gpt35"] = OpenAIService(
                    model_name="gpt-3.5-turbo"
                )
                self.service_health["openai_gpt4"] = ServiceHealth.HEALTHY
                self.service_health["openai_gpt35"] = ServiceHealth.HEALTHY
            ...
            if settings.anthropic_api_key:
                self.services["claude_sonnet"] = ClaudeService(
                    model_name="claude-3-sonnet-20240229"
                )
                self.services["claude_haiku"] = ClaudeService(
                    model_name="claude-3-haiku-20240307"
                )
                self.service_health["claude_sonnet"] = ServiceHealth.HEALTHY
                self.service_health["claude_haiku"] = ServiceHealth.HEALTHY
                logger.info("Claude services initialized")
            else:
            ...
            if not self.services:
                raise AIServiceError("No AI services could be initialized", "Orchestrator")
            ...
            raise AIServiceError(f"AI Orchestrator initialization failed: {str(e)}", "Orchestrator")
    ...
    def _select_service(
        ...
        if strategy == RoutingStrategy.SPECIFIC_MODEL and preferred_service:
            return preferred_service if preferred_service in available_services else None
    ...
    def _performance_based_selection(self, services: List[str], request: AIRequest) -> str:
        ...
        for service_name in services:
            ...
            response_time_score = max(0, 1 - (metrics["avg_response_time"] / 30))  # 30s max
    ...
    def _cost_optimized_selection(self, services: List[str], request: AIRequest) -> str:
        ...
        if request.task_type in [AITaskType.SENTIMENT_ANALYSIS, AITaskType.CONTENT_MODERATION]:
            cheap_services = [s for s in services if "gpt35" in s or "haiku" in s]
            if cheap_services:
                return min(cheap_services, key=lambda s: self.service_metrics[s]["avg_cost"])
    ...
    def _quality_optimized_selection(self, services: List[str], request: AIRequest) -> str:
        """Select service optimized for quality."""
        
        # For creative tasks, prefer more capable models
        if request.task_type in [AITaskType.QUOTE_GENERATION, AITaskType.PSYCHOLOGY_ANALYSIS]:
            premium_services = [s for s in services if "gpt4" in s or "sonnet" in s or "opus" in s]
            if premium_services:
                return max(premium_services, key=lambda s: self.service_metrics[s]["avg_quality_score"])
        
        # Select based on quality score
        return max(services, key=lambda s: self.service_metrics[s]["avg_quality_score"])
    
    async def _try_fallback(self, request: AIRequest, failed_service: str) -> AIResponse:
        """Try fallback services when primary service fails."""
        
        available_services = [
            name for name in self._get_healthy_services()
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        return {
            "overall_health": "healthy" if any(
                h == ServiceHealth.HEALTHY for h in self.service_health.values()
            ) else "degraded",
    ...
    def get_service_stats(self) -> Dict[str, Any]:
        ...
        total_successful = sum(m["successful_requests"] for m in self.service_metrics.values())
    ...
    async def generate_quote(
        ...
        **kwargs
        ...
        strategy = RoutingStrategy.SPECIFIC_MODEL if preferred_model else RoutingStrategy.QUALITY_OPTIMIZED
