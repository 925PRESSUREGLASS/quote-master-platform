...
from src.services.ai.agents.enhanced_orchestrator import EnhancedAIOrchestrator, AgentRequest
from src.services.ai.agents.agent_types import AgentRole, AgentContext, TaskComplexity
from src.services.ai.agents.agent_workflows import AgentWorkflowManager, get_workflow_manager
...
async def test_agent_system():
    ...
    try:
        ...
        while total_waited < max_wait_seconds:
            ...
            if status:
                print(f"⏳ Workflow Status: {status['status']} - Step {status['completed_steps']}/{status['total_steps']}")
        ...
        strategies = [TokenStrategy.COST_MINIMIZED, TokenStrategy.BALANCED, TokenStrategy.QUALITY_FIRST]
        ...
        for strategy in strategies:
            ...
            if result["success"]:
                print(f"  ✅ {strategy.value}: ${result['metrics']['cost_usd']:.4f} | {result['metrics']['tokens_used']} tokens")
            else:
        ...
        print(f"🔢 Total Agent Requests: {orchestrator_status['agent_metrics']['total_requests']}")
        print(f"📊 Active Workflows: {orchestrator_status['agent_metrics']['active_workflows']}")
        print(f"🎯 Available Agents: {len(orchestrator_status['capabilities']['available_agents'])}")
        print(f"🏗️ Available Tiers: {len(orchestrator_status['capabilities']['available_tiers'])}")
